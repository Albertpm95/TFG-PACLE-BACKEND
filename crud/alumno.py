...
def create_alumno(alumno: sch_alumno, db: Session):
    ...
    if not existe_genero:
        raise HTTPException(
            status_code=404, detail="No se encuentra el Genero seleccionado en la DB, no puede registrarse al alumno."
        )
    ...
    if not existe_colectivoUV:
        raise HTTPException(
            status_code=404, detail="No existe ese Colectivo UV seleccionado, no puede registrarse al alumno."
        )
...
def update_alumno(alumno: sch_alumnoDB, db: Session):
    ...
    if not existe_alumno:
        raise HTTPException(
            status_code=404, detail="No existe el alumno, no se pueden actualizar sus detalles."
        )
    existe_genero: Genero = crud_genero.get_genero_id(db, alumno.genero.idGenero)
    if not existe_genero:
        raise HTTPException(
            status_code=404, detail="No se encuentra el Genero seleccionado en la DB, no puede actualizarse al alumno."
        )
    existe_colectivoUV: ColectivoUV = crud_colectivoUV.get_colectivoUV_id(db, alumno.colectivoUV.idColectivoUV)
    if not existe_colectivoUV:
        raise HTTPException(
            status_code=404, detail="No existe ese Colectivo UV seleccionado, no puede actualizarse al alumno."
        )
